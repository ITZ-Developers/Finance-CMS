...
const UserNotificationListPage = ({ pageOptions }) => {
            ...
            render(dataRow) {
                return (
                    <div style={{ fontWeight: 'normal' }}>
                        {convertUtcToLocalTime(dataRow?.account?.birthDate, DEFAULT_FORMAT, DATE_FORMAT_DISPLAY)}
                    </div>
                );
            },
        },
    ...
    return (
        <PageWrapper routes={pageOptions.renderBreadcrumbs(commonMessage, translate, `?serviceId=${serviceId}` )}>
            <ListPage
