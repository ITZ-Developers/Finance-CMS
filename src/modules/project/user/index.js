...
const UserProjectListPage = ({ pageOptions }) => {
    ...
    const { data, mixinFuncs, queryFilter, loading, pagination, serializeParams } = useListBase({
        apiConfig: {
            getList: apiConfig.taskPermission.getList,
            getById: apiConfig.taskPermission.getById,
            update: apiConfig.taskPermission.updateAdmin,
            delete: apiConfig.taskPermission.delete,
        },
        options: {
    ...
    return (
        <div>
            <PageWrapper routes={pageOptions.renderBreadcrumbs(commonMessage, translate)}>
                <ListPage
                    title={<span style={{ paddingTop: 20, display: 'block' }}>Project name: {projectName}</span>}
            ...
            <ModalPermissionCreate
                ...
                title="Select user to view project"
                name="accountId"
                label={'Account'}
